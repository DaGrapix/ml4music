...
#selection des indices de la matrice de correlation > 0.75
high.corr.index.new <- which(corr > 0.75, arr.ind = TRUE) %>% unname
(nrow(high.corr.index.new) - p - 1)/2
```
141 ont une corrélation supérieure à 75%.
...
train = sample(c(TRUE,FALSE), n, rep=TRUE, prob=c(2/3,1/3))
x.train <- x[which(train),] %>% as.matrix()
x.test <- x[which(train==FALSE),] %>% as.matrix()
